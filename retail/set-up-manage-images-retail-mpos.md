---
title: "Saate seadistada ja hallata pilte jaemüügi kaasaegne POS"
description: "Selles artiklis kirjeldatakse meetmeid, mis tegelevad loomine ja haldamine erinevate üksuste ja jaemüügi kaasaegne POS (MPOS) kuvatavaid pilte."
author: MargoC
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="set-up-and-manage-images-for-retail-modern-pos"></a>Saate seadistada ja hallata pilte jaemüügi kaasaegne POS

Selles artiklis kirjeldatakse meetmeid, mis tegelevad loomine ja haldamine erinevate üksuste ja jaemüügi kaasaegne POS (MPOS) kuvatavaid pilte.
...
Pilte, mis on jaemüügi kaasaegne POS (MPOS) peavad olema majutatud väliselt, väljaspool Microsoft Dynamics 365 toiminguteks - jaemüük. Tavaliselt majutatakse neid sisuhaldussüsteemis, sisu edastamise võrgus (CDN) või meediserveris. MPOS toob pildid sobivate üksuste (nagu tooted ja kataloogid) juurde ning kuvab need, kasutades sihtkoha URL-i. Väliselt majutatavate piltide toomiseks nõuab MPOS piltide õiget URL-i vormingut. Saate konfigureerida piltidele nõutava URL-i vormingu, seadistades kanali profiilis väärtuse **Meedia baas-URL **ja kasutades iga üksuse puhul funktsiooni **Meediamalli määratlemine**. Samuti saate alistada standardse URL-i vormingu üksuste alamkogumi jaoks, kasutades funktsiooni **Excelis redigeerimine**. **Tähtis märkus:** Dynamics 365 operatsioonide praeguses versioonis enam seadistage URL formaadis kasutades on **pilt** atribuut XML MPOS ja selle **vaikimisi** atribuuti rühma üksused. Kui olete tuttav Microsoft Dynamics AX 2012 R3 ja juba kasutab praeguse versiooni Dynamics 365 toiminguteks, veenduge, et te alati kasutada uute **Määratle media malli** funktsionaalsus: luua pilte. Ärge kasutage ega muutke ühegi üksuse (sh tootede) puhul atribuuti **Pilt** atribuudigrupis **Vaikimisi**. Otse atribuudigrupis **Vaikimisi** piltidele tehtud muudatusi ei kajastata. See suvand keelatakse tulevases väljaandes. Järgmistes protseduurides seadistatakse näitena pildid kataloogiüksuse jaoks. Need protseduurid aitavad tagada, et vaikimisi seadistatakse ühist teed kasutavatele kataloogi piltidele õige pildi sihtkoha tee. Näiteks kui olete seadistanud meediaserveri või CDN-i väliselt ja soovite MPOS-is pilte kuvada konkreetse poe jaoks, aitab funktsioon **Meediamalli määratlemine** määrata tee, mille puhul saab MPOS pilte otsida ja neid tuua. **Märkus.** Antud demonandmete näites juurutatakse meediaserver jaemüügiserveris. Aga sul võib olla see kusagil väljaspool Dynamics 365 toiminguteks.

### <a name="set-up-the-media-base-url-for-a-channel"></a>Meedia baas-URL.i seadistamine kanalile

1.  Avage Dynamics 365 toimingute HQ portaali.
2.  Klõpsake **hulgi- ja kaubanduse**&gt;**kanali seaded**&gt;**profiilid kanal**. [![kanali profile1](./media/channel-profile1.png)](./media/channel-profile1.png)
3.  Värskendage kanali profiilis, mida teie kauplus MPOS-ide jaoks kasutab, välja **Meedia baas-URL** oma meediaserveri või CDN-i baas-URL-iga. Baasi URL on esimene osa URL, mida jagavad kõik pilt kaustadesse eri üksuste. [![kanal-profile2](./media/channel-profile2.png)](./media/channel-profile2.png)

### <a name="define-the-media-template-for-an-entity"></a>Üksusele meediamalli määratlemine

1.  Klõpsake **hulgi- ja kaubanduse**&gt;**kataloogi juhtimise**&gt;**kataloogi Pildid**.
...
9.  Kataloogi Pildid kanal pool meedia malli värskendamiseks käivitage kindlasti **kataloogi töö 1150** : **jaemüügi see**&gt;**jaotus ajakava**. [![catalog1](./media/catalog1.png)](./media/catalog1.png)

## <a name="previewing-an-image-from-the-entity-level"></a>Pildi eelvaatamine üksuse tasemelt
1.  HQ üksuse lehelt saate eelvaadata pilti, mis kasutab pildi URL-i, mis on tuletatud meediamallist. Selle näiteks avage asjakohane kataloog ja Updatehagi paani, klõpsake **Media**&gt;**pildid**. Kasutage ripploendit, et valida erinevaid kauplusi, millel võivad olla erinevad kanali profiilid.
...
Kõik viis toetatud ettevõtted (töötaja, kliendi, kataloogi, kategooria ja toodete), kui olete selle **Määratle media malli** dialoog pilti luua, veenduge, et te töö kataloog (1150) **jaemüügi see**&gt;**jaotus ajakava**. See töö võimaldab värskendatud meediamalli kanaliga sünkroonimist ja lubab MPOS-il seda kasutada. Käivitage kataloogitöö (1150) pärast mõne järgmise muudatuse tegemist.

-   Värskendate kataloogi pildi meedia mall: **kataloogi Pildid**&gt;**Määratle media malli**.
-   Värskendate töötaja pilt media mall: **töötaja pildid**&gt;**Määratle media malli**.
-   Värskendamist: pildi meedia kliendimalli **kliendi pilt**&gt;**Määratle media malli**.
-   Olete toote pildi meedia mudel: **toote pildid**&gt;**Määratle media malli**.
-   Värskendamist: malli kategooria pilt meedia **kategooria pildid**&gt;**Määratle media malli**. Peate avaldama ka kanali.
...
4.  Klõpsake kiirkaardil **Pildi URL-ide loomine Exceli jaoks** suvandit **Loo**. **Oluline.** Alati kui meediamalli muudetakse, peate klõpsama suvandit **Loo**, enne kui saate kasutada funktsiooni Excelis redigeerimine. [![excel1](./media/excel1.jpg)](./media/excel1.jpg) nüüd pildi URL, tekkinud viimati salvestatud meedia Mall eelvaates vaadata. [![excel2](./media/excel2.png)](./media/excel2.png)**Märkus:** The URL-id, loodud Exceli kasutada tee ja konventsioonides määratletud media malli. Tavad hõlmavad tavasid failinimede osas. Eeldus on, et olete seadistanud füüsilised pildid väljaspool Dynamics AX-i ja pilte saab tuua URL-idest, mis on tuletatud varem määratletud meediamallist. Saate need tuletatud URL-id üle kirjutada, kasutades funktsiooni Excelis redigeerimine.
...
7.  Kui küsitakse, klõpsake parempoolsel paanil suvandit **Usalda seda lisandmoodulit** ja oodake, et lisandmoodul installimise lõpetaks. [![Usalda see lisandmoodul](./media/excel4.jpg)](./media/excel4.jpg)
8.  Kui teil palutakse sisse logida, sisestage mandaadid, mida kasutasite HQ-sse sisselogimiseks. [![Sisselogimisviip](./media/excel5.png)](./media/excel5.png)
...
11. Kõikide üksuste puhul (v.a Tooted) saate pildi-URL-id üle kirjutada. Muutke olemasolevat pildi-URL-i nii, et see kasutab pildi uut sihtkoha URL-i, ja värskendage pildifaili failinime. Faili nimi peab olema kordumatu, mis tagab, et kirje on kordumatu. [![Kirjuta pildi URL Excelis](./media/excel6.jpg)](./media/excel6.jpg)**Märkus:** ülekirjutamisel Redigeeri abil Exceli funktsioonid või üksus üksuse lehele pildi URL-e toodete jaoks, MPOS alati näitab kogu meedia malli pildi URL-id koos ülekirjutatud pildi URL-id.
...
1.  Klõpsake **kataloogid**&gt;**meedia**&gt;**pildid**, ja valige kataloogi pildi värskendamiseks.
...
**[![Cat3](./media/cat3.png)](./media/cat3.png)Märkus:** pärast kategooria pilt association, tuleb avaldada kanal ja kanal töö tagamiseks, et muudatused avaldatakse kanali andmebaasi.
...
1.  Klõpsake **toote teabekorraldus**&gt;**toodete**&gt;**toodete**.
...
1.  Mine kataloogi ja Updatehagi paani, klõpsake **Media**&gt;**pildid**.
