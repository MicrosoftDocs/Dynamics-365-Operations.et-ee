---
title: Saate vaadata sisseelavate töötajate olekut ja oma tööülesandeid
description: 'Selles teemas selgitatakse, kuidas kasutada rakendust Microsoft Dynamics 365 Talent: Onboard, et jälgida, kus uued töötajad oma sisseelamisprotsessiga on.'
author: andreabichsel
manager: ''
ms.date: 05/02/2019
ms.topic: article
ms.prod: ''
ms.service: dynamics-365-talent
ms.technology: ''
ms.search.form: HcmCourseType, HcmCourseTypeGroup, HRMCourseTable
audience: Application User
ms.reviewer: anbichse
ms.search.scope: Core, Operations, Talent
ms.search.region: Global
ms.author: anbichse
ms.search.validFrom: 2019-05-06
ms.dyn365.ops.version: Talent
ms.openlocfilehash: 2039c09372381ef2c078e1f4d4ed8a25c22de5c7
ms.sourcegitcommit: 199848e78df5cb7c439b001bdbe1ece963593cdb
ms.translationtype: HT
ms.contentlocale: et-EE
ms.lasthandoff: 10/13/2020
ms.locfileid: "4460970"
---
# <a name="view-the-status-of-onboarding-employees-and-your-tasks"></a>Saate vaadata sisseelavate töötajate olekut ja oma tööülesandeid

[!include [banner](includes/banner.md)]

Microsoft Dynamics 365 Talent: Onboard võimaldab vaadata edusamme, mida uued töötajad oma sisseelamisprotsessis teevad. Samuti saate vaadata kõiki ülesandeid, mis on teile sisseelamisjuhenditest määratud.

## <a name="view-the-status-of-onboarding-employees"></a>Vaadake töötajate töölevõtmise olekut

1. Valige vasakpoolses menüüs **Juhendid**. Näete ülevaadet kõigist teie saadetud sisseelamisjuhendite edenemisest.

    ![[Saate vaadata töölevõtmisjuhendite olekut](./media/onboard-guide-status.png)](./media/onboard-guide-status.png)

2. Süvitsiminekuks valige juhend, mida soovite vaadata.
3. Valige vahekaart **Tegevused**, et näha, millised tegevused teie uus töötaja on lõpule viinud.

    ![[Saate vaadata toiminguid, mis palgatud töövõtja on lõpule viinud](./media/onboard-status-activities.png)](./media/onboard-status-activities.png)

## <a name="view-onboarding-tasks-that-are-assigned-to-you"></a>Vaadake teile määratud sisseelamisülesandeid

1. Valige vasakpoolses menüüs **Ülesanded**. Näete ülevaadet kõigist selle kohta tehtud toimingutest, mis on teile määratud töötajatele saadetud sisseelamisjuhenditest.
2. Süvitsimineku valimiseks valige soovitud toiming.
3. Järgige vajadusel uut töötajat.
4. Kui olete tööülesande täitnud, märkige see lõpetatuks, lisades sellele ringile märke.

## <a name="next-steps"></a>Järgmised etapid

- [Looge Onboardis värbamismeeskonnad](./onboard-create-team.md)

### <a name="see-also"></a>Vt ka

- [Proovige või ostke Onboardi rakendus](https://dynamics.microsoft.com/talent/onboard/)
- [Mis on uut või mida on muudetud rakenduses Dynamics 365 Talent?](./whats-new.md)
- [Väljaandmisplaanid](https://docs.microsoft.com/business-applications-release-notes/index)
- [Toe hankimine rakendusele Microsoft Dynamics 365 Talent](./talent-support.md)
