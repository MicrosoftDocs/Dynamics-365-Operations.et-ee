...
- Mõistate, kuidas saate tegevuse salvestaja funktsiooniga rakenduses Microsoft Dynamics 365 for Finance and Operations koos teenustega Microsoft Dynamics Lifecycle Services (LCS) ja Microsoft Azure DevOps luua, konfigureerida, käitada ja uurida testjuhtumeid ning nende kohta aruandeid esitada.
- Volitage funktsiooni kasutajaid kirjendama äriülesandeid, kasutades tegevuse salvestajat rakenduses Finance and Operations, ja seejärel teisendage ülesande salvestised automaatsete katsete komplekti, ilma et peaksite kirjutama lähtekoodi.

## <a name="before-you-begin"></a>Enne alustamist

### <a name="prerequisites"></a>Eeltingimused

- Selle õppetüki jaoks on vaja Finance and Operationsi keskkonda, kus töötab Microsoft Dynamics 365 for Finance and Operationsi versioon 10.0 (aprill 2019) või uuem versioon. Klientide jaoks, kes kasutavad Microsoft Dynamics 365 for Finance and Operationsi, Enterprise edition 7.3, toetatakse ka platvormivärskendust 20 (PU20) või uuemat versiooni.
- Kasutajal peavad olema Finance and Operationsi keskkonnas administraatoriõigused.
...
## <a name="finance-and-operations-environment"></a>Finance and Operationsi keskkond

### <a name="version-requirement"></a>Nõudmised versioonile

Vajalik on Finance and Operationsi katse- või liivakastikeskkond versiooniga 10. Klienride jaoks, kes kasutavad versiooni 7.3, toetatakse ka platvormivärskendust 20 või uuemat versiooni.

> [!NOTE]
> RSAT-l peab olema juurdepääs Finance and Operationsi katsekeskkonnale veebibrauseri kaudu.
...
See etapp on vajalik ühenduse loomiseks LCS-iga, et tegevuse salvestisi saaks salvestada vastavasse BPM-i teeki LCS-is Finance and Operationsi kliendi kaudu.

1. Avage Finance and Operationsi klient.
...
> [!NOTE]
> Veenduge, et kõik tegevuse salvestised algaksid Finance and Operationsi peamiselt armatuurlaualt. Hoidke üksikud salvestised lühikesed ja keskenduge ühele äriülesandele, näiteks müügitellimuse loomisele.
...
RSAT-d saab installida igasse arvutisse, kus töötab Windows 10 ja mille saab ühendada veebibrauseri kaudu (Internet Explorer või Google Chrome) Finance and Operationsi keskkonnaga.
...
> [!NOTE]
> Kui Finance and Operationsi keskkond on järgu 2+ keskkond, tuleb serdi sõrmejälge värskendada failis wif.config keskkonna **kõikide** AOS-i arvutite jaoks.
...
4. Värskendage faili **wif.config**, lisades serdile ja ameti nimele uus asutuse kirje, nagu on näidatud järgmises näites.
    ...
    > [!NOTE]
    > Kui mitu kasutajat kasutab sama Finance and Operationsi rakendust, peab iga kasutaja looma eraldi sõrmejäljed, mis tuleb kõik lisada jaotisesse **\<võtmed\>**.
...
Kui kasutatakse järgu 2+ (standardse vastuvõtutesti liivakast või uuem versioon) Finance and Operationsi keskkonda, siis kontrollige järgmist registri sätet või seadistage see arvutis, kuhu on installitud RSAT. See etapp on vajalik, kuna see aitab vältida autentimise või RSAT ühenduse tõrkeid.
...
### <a name="configure-rsat"></a>Konfigureeri RSAT
...
3. Valige nupp **Sätted** (hammasratta sümbol) ja seejärel täitke ilmuvas dialoogiaknas järgmised väljad.
    ...
    - **Hosti nimi** – sisestage Finance and Operationsi katsekeskkonna hosti nimi, näiteks **\<myaos\>.cloudax.dynamics.com**. Ärge lisage eesliidet **https://** või **http://**.
    - **SOAP-i hosti nimi** – sisestage Finance and Operationsi katsekeskkonna SOAP-i hosti nimi. Tavaliselt on SOAP-i hosti nimi sama kui hosti nimi, aga sellel on järelliide **soap**. Siin on näide: **\<myaos\>soap.cloudax.dynamics.com**. Ärge lisage eesliidet **https://** või **http://**.
    ...
    - **Administraatori kasutajanimi** – sisestage Finance and Operationsi katsekeskkonda administraatori meiliaadress.
    ...
    - **Ettevõtte nimi** – sisestage ettevõtte nimi, mida kasutatakse Exceli parameetri faili loomisel Finance and Operationsi vaikimisi ettevõttena. Saate ettevõtet hiljem muuta, redigeerides Exceli parameetrifaili.
...
3. Iga käivituse jaoks kordumatu tootenumbri loomiseks Exceli parameetrifaili uuesti avamata ja tootenumbrit iga kord käsitsi värskendamata kasutage järgmist Exceli valemit.
    ...
    > [!NOTE]
    > Lisaks vahekaardile **Üldine** sisaldab Exceli parameetrifail andmete vahekaarti iga Finance and Operationsi vormi lehe kohta, mida testjuhtum külastab.
