...
Selles teemas loote nõutavad ER konfiguratsioonid näidisettevõttele Litware, Inc. Seetõttu peate enne selles teemas protseduuride sooritamist neid samme järgima.

1. Avage **Organisatsiooni haldamine** \> **Tööruumid** \> **Elektrooniline aruandlus**.
2. Lokalisatsiooni konfiguratsioonide lehel kontrollige, et konfiguratsioonipakkuja **·** **Litware, Inc.** näidisettevõttele on saadaval ja märgitud aktiivsena. Kui te ei näe seda konfiguratsioonipakkujat, täitke esmalt sammud jaotises Loo [konfiguratsioonipakkujad ja märkige need](er-configuration-provider-mark-it-active-2016-11.md) aktiivseks.
...
7. Valige **[...](../er-download-configurations-global-repo.md#open-configurations-repository)** avatud, et vaadata läbi valitud hoidla ER-konfiguratsioonide loend.
...
3. Ilmuvad dialoogiaknas tehke järgmist.
    ...
    2. Väljale **Nimi** sisestage **·** Juur.
    3. Väljal **·** Erimärgid valige Uus rida - Windows **(CR CR)**. Selle sätte alusel peetakse iga rida sõelumisfailis eraldi kirjeks.
    4. Valige nupp **OK**.
...
6. Valige puus **juurread \\ ja seejärel käsk Lisa** **seeria**.
...
2. Valige **tegevuspaanil mudeli** ja andmeallika vastendamise lehel suvand **·** Uus.
3. **Definitsiooniväljal** valige **BankToCustomerDebitCreditNotificationInitiation.**
4. Sisestage **väljale Nimi väärtus** **Vastendamine andmemudeliga.**
5. Valige käsk **Salvesta**.
6. Valige **Kujundaja**.
7. Valige **mudeli vastendamise** kujundaja lehel **andmeallikate tüüpide** puus väärtus **Dynamics 365 for Operations\\** Klass.
...
9. Ilmuvad dialoogiaknas tehke järgmist.
    ...
    1. Väljale **Nimi** sisestage **\_ Tšekikoodid.**
...
10. Andmeallikate **tüüpide** puus järgige neid samme.
    ...
    3. Laienda **\\ vorming Juur: seeria(juur): \\ seeria 1.\* (read)**
    4. Laiendage **vormingu \\ juur: seeria(juur)read: \\ seeria 1.\* (read) \\ väljad: seeria 1..1** (väljad).
...
11. Andmemudeli **·** puus järgige neid samme.
    ...
    4. Laiendage **maksete \\ kreeditkonto(CreditorAccount) \\\\ IBAN-i** ID.

12. Järgige neid samme konfigureeritud vormingu komponentide sidumiseks andmemudeli väljadega.

    1. Valige **vormingu \\ juur: seeria(juur)read: \\ seeria 1.\* (read)**
    2. Valige **·** maksed.
    ...
    5. Valige **maksete \\ kreeditkonto (CreditorAccount) \\\\** IBAN.
...
13. Vahekaardil Kontrollimised järgige neid samme, et lisada kinnitusreegel, mis näitab tõrketeadet mis tahes real sõelumisfailis, mis sisaldab kehtetut **·**[...](../general-electronic-reporting-formula-designer.md#Validation) IBAN-koodi:

    1. Valige **väärtus Uus ja seejärel käsk Redigeeri** **·** tingimust.
    2. Laiendage valemikoosturi lehel andmeallika puus kontrollkoodide andmeallikat, mis esindab **·** **·** **ISO7064 rakendusklassi, et vaadata selle klassi \_** **·** saadaolevaid meetodeid.
    3. Valige **\_\\ kontrollkoodid verifyMOD1271 \_ 36.**
    4. Valige **Andmeallika lisamine**.
    5. Sisestage **valemi väljale järgmine**[avaldis:](../general-electronic-reporting-formula-designer.md#Binding)**kontrollige \_ koode.verifyMOD1271 \_ 36(vorming. Root.Rows.Fields.IBAN)**.
    6. Valige **Salvesta** ja sulgege seejärel leht.
    7. Valige **suvand Redigeeri** teadet.
    8. Sisestage **·** valemikoosturi lehe **valemiväljal** **CONCATENATE("Kehtetu IBAN-kood: &nbsp; ", vorming. Root.Rows.Fields.IBAN)**.
    9. Valige **Salvesta** ja sulgege seejärel leht.

    Nende sätete alusel annab kinnitamistingimus iga kehtetu IBAN-koodi puhul väärtuse FALSE, kutsudes *[...](../er-formula-supported-data-types-primitive.md#boolean)* olemasoleva **VERIFYMOD1271 \_ 36** meetodi **ISO7064** rakenduseklassile. Pange tähele, et IBAN-koodi väärtus määratletakse käitusajal dünaamiliselt helistamismeetodi argumendina, mis põhineb sõelumisteksti faili sisul.
...
2. Elektroonilise aruande parameetrite lehel valige Sirvi, sirvige **·** allalaaditud faili **·** **SampleIncomingMessage.txt ja** valige see.
3. Valige nupp **OK**.
