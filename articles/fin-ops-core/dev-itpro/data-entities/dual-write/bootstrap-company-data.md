...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="cac07-103">Alglaadimine ettevõtte andmetega, KKK</span><span class="sxs-lookup"><span data-stu-id="cac07-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]


## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="cac07-104">Milleks on vaja alglaadimist?</span><span class="sxs-lookup"><span data-stu-id="cac07-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="cac07-105">Teil võib olla olemasolev Common Data Service’i või muu Dynamics 365 rakenduse ettevõtte andmetega eksemplar ja soovite selle jaoks lubada topeltkirjutamise ühendust.</span><span class="sxs-lookup"><span data-stu-id="cac07-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="cac07-106">Sellisel juhul peate alglaadima Common Data Service’i või muu Dynamics 365 rakenduse andmeid ettevõtte teabega enne topeltkirjutamise ühenduse lubamist.</span><span class="sxs-lookup"><span data-stu-id="cac07-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="cac07-107">Millal peaksin alglaadimist kasutama?</span><span class="sxs-lookup"><span data-stu-id="cac07-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="cac07-108">Kasutage alglaadimist enne topeltkirjutamise olemikaartide lubamist (etapis 5).</span><span class="sxs-lookup"><span data-stu-id="cac07-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="cac07-109">Finance and Operations rakenduse ja Common Data Service’i või muu Dynamics 365 rakenduse vahel topeltkirjutamise ühenduse seadistamiseks logige administraatorina sisse Finance and Operationsi rakendusse.</span><span class="sxs-lookup"><span data-stu-id="cac07-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="cac07-110">Minge moodulisse **Andmehaldus** ja klõpsake nuppu **Topeltkirjutamine**.</span><span class="sxs-lookup"><span data-stu-id="cac07-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="cac07-111">See käivitab **andmeintegraatori**.</span><span class="sxs-lookup"><span data-stu-id="cac07-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="cac07-112">Looge topeltkirjutamise ühendus ühele või mitmele ettevõttele.</span><span class="sxs-lookup"><span data-stu-id="cac07-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="cac07-113">![Topeltkirjutamise ühenduse loomine](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="cac07-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="cac07-114">Lubage olemikaart **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="cac07-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="cac07-115">See sünkroonib ettevõtted rakendusest Finance and Operations rakendusse Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="cac07-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="cac07-116">![Olemikaardi lubamine](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="cac07-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="cac07-117">Käivitage alglaadimise näidiskood Common Data Service’is või muus Dynamics 365 rakenduse eksemplaris.</span><span class="sxs-lookup"><span data-stu-id="cac07-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="cac07-118">Kui alglaadimine on tehtud ja süsteem on reaalajas sünkroonimiseks valmis, lubage olemikaardid.</span><span class="sxs-lookup"><span data-stu-id="cac07-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="cac07-119">Olemikaartide lubamine käivitab andmete algse sünkroonimise lubatud olemikaartide jaoks.</span><span class="sxs-lookup"><span data-stu-id="cac07-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="cac07-120">Topeltkirjutamise ühenduses valitud ettevõtetele vastavad andmed sünkroonitakse rakenduste Finance and Operations ja Common Data Service vahel.</span><span class="sxs-lookup"><span data-stu-id="cac07-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="cac07-121">Kui kasutada näidiskoodi?</span><span class="sxs-lookup"><span data-stu-id="cac07-121">How to I use the code sample?</span></span>
<span data-ttu-id="cac07-122">Näidiskood on C# rakendus, mille saate laadida rakenduses Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="cac07-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="cac07-123">See võtab NuGeti paketi sõltuvused Common Data Service’i SDK-s, mida saate värskendada Visual Studio standardsete tööriistadega.</span><span class="sxs-lookup"><span data-stu-id="cac07-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="cac07-124">Pärast lahenduse lahtipakkimist ja avamist Visual Studios ning NuGeti pakettide taastamist otsige koodist väljendit **TODO**.</span><span class="sxs-lookup"><span data-stu-id="cac07-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="cac07-125">Iga otsus, mille peate tegema selle kohta, kuidas soovite ettevõtte teavet alglaadida, on tähistatud väljendiga **TODO** ja näidiskoodiga kanoonilise juurutuse jaoks.</span><span class="sxs-lookup"><span data-stu-id="cac07-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO** , with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="cac07-126">Näidiskood näitab vaid üht mitmest viisist, kuidas olemikirjeid ettevõtte järgi liigitada.</span><span class="sxs-lookup"><span data-stu-id="cac07-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="cac07-127">Muutes loogikat väljendiga **TODO** tähistatud jaotistes, saate luua kohandatud liigituse.</span><span class="sxs-lookup"><span data-stu-id="cac07-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="cac07-128">Mida peaksin ootama?</span><span class="sxs-lookup"><span data-stu-id="cac07-128">What should I expect?</span></span>
<span data-ttu-id="cac07-129">Vaikimisi laseb näidisrakendus sisestada äriüksuse-ettevõtte koodi vastenduste sõnastiku.</span><span class="sxs-lookup"><span data-stu-id="cac07-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="cac07-130">Üksus, mida alglaadite väljaga **OwningBusinessUnit** , seadistatakse automaatselt kasutama määratud ettevõtet.</span><span class="sxs-lookup"><span data-stu-id="cac07-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="cac07-131">Üksus ilma väljata **OwningBusinessUnit** , nt toode, seadistab ettevõtte vastenduse põhjal tühja äriüksuse väärtusega.</span><span class="sxs-lookup"><span data-stu-id="cac07-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="cac07-132">Konsooli rakendus eeldab üht parameetrit, kas **–simuleeri** või **–rakenda**.</span><span class="sxs-lookup"><span data-stu-id="cac07-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="cac07-133">Kui kasutate käsurea parameetrit **–simuleeri** , siis andmeid ei värskendata.</span><span class="sxs-lookup"><span data-stu-id="cac07-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="cac07-134">Ainult failid **simulation_<entityname>.csv** luuakse samas kataloogis kui tööriist, üks iga üksuse kohta, mida oleks värskendatud.</span><span class="sxs-lookup"><span data-stu-id="cac07-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="cac07-135">Saate neid faile korduvalt üle vaadata, tagades samas, et kood värskendab ettevõtte väärtusi ootuspäraselt.</span><span class="sxs-lookup"><span data-stu-id="cac07-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="cac07-136">Kui olete simuleeritud värskendustega lõpetanud, kasutage parameetrit **–rakenda**.</span><span class="sxs-lookup"><span data-stu-id="cac07-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="cac07-137">See värskendab kõiki kirjeid, millel on parasjagu vale ettevõtte väärtus, korraga 1000 kirjet (vaikimisi).</span><span class="sxs-lookup"><span data-stu-id="cac07-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="cac07-138">Kood on idempotentne, mis tähendab, et saate seda uuesti käitada ja värskendatakse ainult valesti määratud ettevõtteid.</span><span class="sxs-lookup"><span data-stu-id="cac07-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="cac07-139">Käitamisel parameetriga **–rakenda** , väljastab kood tehtud muudatustest CSV-failid, millele antakse nimi **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="cac07-139">When running with **–apply** , the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
