---
title: Vara rentimise avaleht
description: See teema sisaldab ülevaadet vara rentimise dokumentatsioonist ja linke konkreetsetele teemadele.
author: moaamer
manager: Ann Beebe
ms.date: 10/27/2020
ms.topic: article
ms.prod: ''
ms.service: dynamics-ax-applications
ms.technology: ''
ms.search.form: ''
audience: Application User
ms.reviewer: roschlom
ms.search.scope: Core, Operations, Retail
ms.custom: 4464
ms.assetid: 5f89daf1-acc2-4959-b48d-91542fb6bacb
ms.search.region: Global
ms.author: moaamer
ms.search.validFrom: 2020-10-27
ms.dyn365.ops.version: 10.0.14
ms.openlocfilehash: cd32ad1b745d1bfda25abf57330fb1385014e4fc
ms.sourcegitcommit: f0f5545a8ff99583e0131f435d91c64bb68a1c38
ms.translationtype: HT
ms.contentlocale: et-EE
ms.lasthandoff: 10/30/2020
ms.locfileid: "4442582"
---
# <a name="asset-leasing-home-page"></a>Vara rentimise avaleht

[!include [banner](../includes/banner.md)]

See teema loetleb vara rentimise jaoks saadaolevad spikriteemad ja sisaldab linke konkreetsete teemade juurde. 

Vara rentimine on täiustatud võimalus renditavate varade finantskannete haldamiseks, jälgimiseks ja automatiseerimiseks teenuses Microsoft Dynamics 365 Finance. Vara rentimise funktsioon vastab rahvusvahelistele raamatupidamisstandarditele (IFRS 16) ja Ameerika Ühendriikide üldaktsepteeritavate raamatupidamispõhimõtete (GAAP) standarditele (ASC 842). Vara rentimise funktsioon kogub ja töötleb renditavate varade põhiteavet ning aitab luua töölehe sisestusi terve renditava vara elutsükli jooksul alates esialgsest tuvastamisest, igakuistest töölehe sisestustest kuni renditava vara väärtuse langemiseni ja rentimise lõppemiseni.  

## <a name="asset-leasing-topics"></a>Vara rentimise teemad
Vara rentimise konfigureerimise ja kasutamise abistamiseks on saadaval järgmine teema. 

 - [Vara rentimise alustamine](asset-leasing-quick-start.md) – selles teemas kirjeldatakse vara rentimise üldisi võimalusi ning loetletakse ja määratletakse tarkvaras ja dokumentatsioonis kasutatavad terminid.
 
 ### <a name="set-up-asset-leasing"></a>Vara rentimise häälestus
 See teemade rühm aitab teil konfigureerida vara rentimise viisil, mis on teie organisatsiooni äriolukorrale optimaalne.  
  
  - [Rentimise parameetrite konfigureerimine](config-lease-parameters.md) 
  - [Rentimise raamatute häälestamine](set-up-lease-books.md)
  - [Rentimise grupi loomine](create-lease-group.md)
  - [Indeksimäärade seadistamine](set-up-index-rate-types.md)
  - [Kulutüüpide seadistamine](set-up-expense-types.md)
  - [Rentise töölehe nimede seadistamine](set-up-lease-journal-names.md)
  - [Rendi sisestuskontode seadistamine](set-up-lease-posting-accts.md)
  - [Numbriseeriate määramine](leasing-number-sequences.md)
  - [Kasutaja rollide määramine](lease-user-roles.md)

### <a name="manage-asset-leases"></a>Vara rentimiste haldamine
See teemade rühm kirjeldab rentimiste lisamise protsessi, maksearvete loomist, rendikirjete korrigeerimist, rentimise importimise teavet ja kasutamisõiguse esemeks oleva vara kahjustamist. 

 - [Rendikirje lisamine või kopeerimine](add-lease.md)
 - [Maksearve loomine](create-payment-invoice.md)
 - [Kasutamisõiguse esemeks oleva vara kulumi kirjendamine](record-rou-asset-depreciation.md)
 - [Sisestatud renditehingute tühistamine](reverse-posted-lease-trans.md)
 - [Rendikirje korrigeerimine](adjust-lease.md)
 - [Kasutamisõiguse esemeks olevate varade väärtuse langus](impair-rou-asset.md)
 - [Sisestatud renditehingute tühistamine](reverse-posted-lease-trans.md)
 - [Kasutamisõiguse esemeks olevate varade väärtuse langus](impair-rou-asset.md)
 - [Rendi seostamine põhivaraga](associate-lease-with-fixed-asset.md)
 - [Rentimise kirjendamine välisvaluutas](record-leases-foreign-currency.md)
 - [Indeksimääraga seotud rendimaksete ümber arvutamine](revalue-payments-tied-2-index-rate.md)
 - [Rendikohustise lühiajalise osa ümberklassifitseerimine](reclassify-st-lease-liability.md)
 - [Kohustise, vara ja kulu kannete kuvamine](view-asset-transactions.md)
 - [Liitintervalli funktsioon](compound-interval-functionality.md)
 - [Igakuiste töölehe kannete kinnitamine partiis](confirm-payment-schedules-in-batch.md)
 - [Igakuiste töölehe kannete loomine partiis](create-monthly-journals-batch.md)
 - [Topeltaruandlus](dual-reporting.md)
 - [Rendi töövoo seadistamine](set-up-lease-wrkflw.md)
 - [Rendi kinnitamise töövoogude kasutamine](use-create-lease-wrkflw.md)
 - [Rendikirje haldamine rendikirje importimise raamistiku kaudu](manage-leases-thru-imprt-framewrk.md)
 
### <a name="asset-leasing-reporting"></a>Vara rentimise aruandlus
See teema kirjeldab vara rentimiseks saadaolevaid aruandeid. 

 - [Vara rentimise aruanded](asset-leasing-rprts.md)
 

## <a name="additional-resources"></a>Lisaressursid

### <a name="whats-new-and-in-development"></a>Mis on uut ja mis on arendamisel?

Avage [Microsoft Dynamics 365 väljalaskeplaanid](https://go.microsoft.com/fwlink/?linkid=2010158), et näha, milliseid uusi funktsioone on kavandatud. 

### <a name="blogs"></a>Ajaveebid

Arvamused, uudised ja muu teabe leiate [Microsoft Dynamics 365 ajaveebist](https://community.dynamics.com/b/msftdynamicsblog?c=Enterprise) ja [ajaveebist Microsoft Dynamics 365 Finance and Operations - Financials](https://community.dynamics.com/365/financeandoperations/b/financials).

[Microsoft Dynamics Operationsi partnerite kogukonna ajaveeb](https://community.dynamics.com/partner/b/operationspartnercommunityblog) on Microsoft Dynamicsi partnerite jaoks kõikehõlmav ressurss, kust nad saavad teada, mis on uut ja põnevat rakenduses Dynamics 365.

### <a name="videos"></a>Videod

Vaadake õppevideoid, mis on saadaval [Microsoft Dynamics 365 YouTube’i kanalil](https://www.youtube.com/channel/UCJGCg4rB3QSs8y_1FquelBQ). 


[!INCLUDE[footer-include](../../includes/footer-banner.md)]