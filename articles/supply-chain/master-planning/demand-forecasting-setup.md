...
Nõudluse prognoos võib luua ettevõtteülesi prognoose. Ettevõttes Dynamics 365 Supply Chain Management, mis on plaanitud koos, grupeeritakse samasse kontsernisisesesse plaanimisgruppi. Ettevõtte kaupa kauba eraldamisvõtmete määramiseks, mida nõudluse prognoosimiseks arvesse võtta, seostage kauba eraldamisvõti kontsernisisese plaanimisgrupi liikmega.
...
1. Kui kontsernisisese plaanimisgrupi liikmetele ei ole määratud ühtki kauba eraldamisvõtit, arvutatakse nõudluse prognoos vaikimisi kõigile kaupadele, mis on määratud kõikidele kauba eraldamisvõtmetele kõigis ettevõtetes. Täiendavad filtreerimisvalikud ettevõtetele ja kauba **eraldamisvõtmed on saadaval dialoogiboksis Looge statistiline alusprognoos (** koondplaneerimine **\> Nõudluse prognoosimine Looge \> statistiline alusprognoos \>).** Kauba eraldamisvõtmete **määramiseks** valitud kontsernisisese plaanimisgrupi ettevõttele valige ettevõte ja seejärel valige kontsernisisese plaanimisgrupi liikmete kiirkaardil **tööriistaribal** kauba eraldamisvõtmed.
...
Lisateavet Selle kohta, kuidas seadistada Azure'i masina õppeteenust ja seejärel kasutada sellega ühenduse loomiseks seadeid, [vaadake jaotisest Azure Machine Learning Service](#setup-amls).
...
Kui kasutate masina õppet prognoosiplaanimise mudelite loomiseks, kontrollite masina õppevalikuid, seades prognoosi *algoritmi parameetritele väärtused*. Need väärtused saadetakse tarneahela haldustest Azure'i masinaõppimiseks. Kasutage prognoosi **algoritmi parameetrite lehte**, et kontrollida, millised väärtused tuleb esitada ja millised väärtused igal peaks olema.

Nõudluse prognoosimise mudelite jaoks kasutatavate vaikeparameetrite ja väärtuste seadistamiseks minge **\>\> koondplaneerimise häälestage nõudluse prognoosi prognoosimise prognoosi \> algoritmi parameetrid.** Antakse standardne parameetrite kogum. Igal parameetril on järgmised väljad:
...
- **Hooajalisus ( Force seasonality**) – määrab, kas mudel peab kasutama kindlat tüüpi hooajalist. See parameeter kehtib ainult ARIMA ja ETS-i puhul. Valikud: *AUTOMAATNE (vaikimisi)*, *POLE*, LISALISAND *·*, *KORRUTATAV*.
...
- **Puuduva väärtuse asendamine** - Määrab, kuidas varasemate andmete lüngad täidetakse. Valikud: *(arvuline väärtus)*, KESKMINE *·*, EELMINE *·*, *LINKIATE LINEAARNE*, *MATEMAATILINE TEHTEKIRI*.
...
1. Avage GitHubis [Dynamics 365 Supply Chain Management malle nõudluse prognoosimiseks Azure Machine Learningi](https://github.com/microsoft/Dynamics-365-Supply-Chain-Management-Demand-Forecasting-With-Azure-Machine-Learning-Service) hoidlaga (repo) ja laadige alla järgmised failid:
...
1. Teie tööruum on nüüd häälestatud. Jäta ette jaotisest [Azure Machine Learning Service ühenduse parameetrid Tarneahela haldus](#demand-forecast-parameters).
...
Lisateavet vt teemast Tööruumi [loomine](/azure/machine-learning/quickstart-create-resources#create-the-workspace).
...
1. Avage GitHub [Dynamics 365 Supply Chain Management mallid nõudluse prognoosimiseks azure Machine Learningi](https://github.com/microsoft/Dynamics-365-Supply-Chain-Management-Demand-Forecasting-With-Azure-Machine-Learning-Service)**taaspoga ja laadige alla fail sampleInput.csv**.
1. Avage ladustamiskonto, mille lõite sammus [1: looge uus tööruumi](#create-workspace) jaotis.
1. Järgige juhendit [konteineri loomiseks,](/azure/storage/blobs/storage-quickstart-blobs-portal#create-a-container) et luua konteiner nimega *laiendusplan-azureml*.
1. Laadige sammus **1 alla laaditud fail sampleInput.csv** äsja loodud konteinerisse. See fail on vajalik müügivõimaluste avaldamiseks ja katseprognoosi loomiseks. Juhiseid vt blokeerimisbloki [üleslaadimine](/azure/storage/blobs/storage-quickstart-blobs-portal#upload-a-block-blob).
...
1. Looge Uus Azure Bloobi mäluseadme tüübi andmekomplekt, mis osutab azureml azureml *ladustamise* konteinerile, mille lõite 2. sammus *: ladustamisjao* konfigureerimine.[...](#config-storage) (Kui uue andmekaupluse autentimise tüüp on <a0/&) *Konto võti*, sisestage loodud ladustamiskontole konto võti. Juhiseid vt ladustamiskonto [pääsuvõtmete haldamine](/azure/storage/common/storage-account-keys-manage?tabs=azure-portal).)
1. Seadistage uus andme talletamis vaikimisi andme talletuseks, avades selle üksikasjad ja **valides käsu Seadista vaikeandme talletuseks**.
...
1. Avage GitHubis [Dynamics 365 Supply Chain Management malle nõudluse prognoosimiseks rakendusega Azure Machine Learning repo](https://github.com/microsoft/Dynamics-365-Supply-Chain-Management-Demand-Forecasting-With-Azure-Machine-Learning-Service) ja laadige alla järgmised failid:
...
1. Logige oma Azure'i portaali sisse.
1. Registreerige uus avaldus rentnikule Azure Active Directory (Azure AD). Juhiseid vt portaalist [, et luua rakendus ja teenuse Azure AD subjekt, mis pääseb juurde ressurssidele](/azure/active-directory/develop/howto-create-service-principal-portal).
1. Järgige viisardi lõpuleviimisel ekraanil olevaid juhiseid. Kasutage vaikesätteid.
1. Andke oma uuele Active Directory rakendusele juurdepääs järgmistele ressurssidele, mille lõite [jaotises Azure'i seadme õppe](#ml-workspace) loonud. Juhiseid vt Azure'i [rollide määramine Azure'i portaali abil](/azure/role-based-access-control/role-assignments-portal?tabs=current). See samm võimaldab süsteemil importida ja eksportida prognoosimisandmeid ja käivitada masina õppevõimaluste käivitamise tarneahela halduses.
...
1. **Loodud avalduse jaotises Tunnistused** &saladused looge avalduse jaoks saladus. Juhiseid vt teemast Kliendisaladuse [lisamine](/azure/active-directory/develop/quickstart-register-app#add-a-client-secret).
...
1. Seadke vahekaardil **Azure Machine Learning Service** järgmised väljad:
    ...
    - **Rentniku ID – sisestage oma Azure'i rentniku ID**. Tarneahela haldus kasutab seda ID-d autentimiseks Azure'i masinaõppimisteenusega. Oma rentniku ID leiate Azure'i **portaali** Azure AD ülevaate leheküljelt.
