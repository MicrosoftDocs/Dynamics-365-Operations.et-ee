...
-   Veeru definitsiooni päiste ala sisaldab aruandes kuvatavat päiseteksti ja vormingut. Päis saate rakendada ühe veeru andmete, võib ulatuda üle mitme veeru või saate rakendada veergudele tingimuslikud. Veeru määratlust võib sisaldada nii palju veeru päise read, kui vaja. **Märkus.** Veerupäised rakenduvad aruande igale andmeveerule. Aruande päised rakenduvad kogu aruandele. Saate määratleda aruande päiseid aruande definitsiooni vahekaardil **Päised ja jalused**.
...
|-------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| SU                      | Pärsivad allkriipsu veeru, kui kas allkriipsu käsk (**---**) või double allkriipsu käsk (**===**) on kantud readefinitsiooni. Näiteks ei pruugi te soovida protsendiarvutuse loodud summade allakriipsutamist.                                                                        |
...
| Numbriga (@FiscalPeriod)           | Selle veeru puhul määratud rahandusperioodi numbrilise vormi printimine. Kui veerg hõlmab mitut perioodi, prinditakse vahemiku viimane periood.                                                                                                                                   |
| Perioodi kirjeldus (@FiscalPeriodName)  | Finantsandmetes määratud rahandusperioodi kirjelduse printimine.                                                                                                                                                                                                                    |
| Majandusaasta (@FiscalYear)               | Rahandusaasta printimine veeru puhul numbrites.                                                                                                                                                                                                                                            |
...
| Lõppemise kuupäev (@EndDate)                     | Veeru lõppkuupäeva printimine.                                                                                                                                                                                                                                                               |
| Nimi: puu (@UnitName)         | Üksuse nime printimine veerupäises veeru piiramisel aruandluspuu kindla üksuseni.                                                                                                                                                                                     |
| Üksuse kirjeldus (@UnitDesc)            | Üksuse kirjelduse printimine veerupäises veeru piiramisel aruandluspuu kindla üksuseni.                                                                                                                                                                              |
| Broneeri kood (@BookCode)                   | Veerus määratud konteerimiskoodi printimine.                                                                                                                                                                                                                                             |
...
-   Kattuvate veerupäiste määramisel kattuvad need aruandesse printimisel. Aruanne on loodud, kuid kuvatakse järgmine hoiatus on **aruande järjekorra olek** välja: "kasutades baasi veerupäised teiste veerupäised ristuvad ja võib põhjustada kattuva teksti." Näiteks päises veerus B on B baasi + 1 ja päises veerus D on baas + 1 f. Sel juhul Päised prinditakse üksteise peale ja on loetamatu. Väärtuse BASE kasutamisel definitsioonis **Algusvahemik/lõppvahemik** veenduge, et vaatate loodavat aruannet, et näha, kas päised kattuvad.
...
-   Tingimuslik printimise veergude (**P&lt;B**, **P&gt;= B**), tingimisi toestavad päised käitub regioonide komiteele päise määratlust. Näiteks kui tingimus on väär, alustavad kõik järgnevad laiendamise tingimuse puhul ühtivad veerud päise printimist.
...
3.  Valige loendist veeru tüüp. Järgmises tabelis kirjeldatakse eri veeru tüüpe.
    ...
    <tr class="odd">
    <td>ATTR</td>
    <td>Konto või kande atribuudi kuvamine veerus, kui teie raamatupidamissüsteem atribuute toetab. Atribuut, mis tuleb rakendada ühele täielikule kontole, ekstrakdib aluseks oleva konto või kande teabe finantsandmetest. Konto tasemel atribuudid kuvatakse konto ja tehingu tasemel atribuudid kuvada andmeid, mis kande sisestamise ajal. Kui valite <strong>ATTR</strong> veeru tüübiks määrata atribuut kategoorias ning <strong>raamatu kood/atribuut kategooria</strong> detail rea veeru määratlust.</td>
    </tr>
    </tbody>
    </table>
...
| ALUS +\# | Kasutada on aasta \#aasta pärast võrdlusaasta. Näiteks baasaastale järgneva kolmanda aasta kasutamiseks sisestage **BASE+3**. |
| ALUS-\# | Kasutada on aasta \#aasta enne aastat. Näiteks eelmise aasta kasutamiseks sisestage **BASE-1**.                 |
...
| ALUS +\#         | Kasutada perioodi \#pärast baasperioodi. Näiteks baasperioodile järgneva kolmanda perioodi kasutamiseks sisestage **BASE+3**.                                                                                               |
| ALUS-\#         | Kasutada perioodi \#enne baasperioodi. Näiteks eelmise perioodi kasutamiseks sisestage **BASE-1**.                                                                                                                 |
| BASE -\#: alus    | Mitme perioodi kasutamine alates mitmest perioodist enne alusperioodi kuni baasperioodini. Näiteks kolme eelmise perioodi ja baasperioodi kasutamiseks sisestage **BASE-3:BASE**.                                                |
| ALUS: ALUS +\#    | Mitme perioodi kasutamine alates baasperioodist kuni mitme perioodini pärast baasperioodi. Näiteks baasperioodi ja kahe järgmise perioodi kasutamiseks sisestage **BASE:BASE+2**.                                                  |
...
Saate rahandusaasta piire ületada mis tahes perioodi spetsifikatsioonis ja kombineerida perioodide vahemiku aastaid. Näiteks määrate perioode nagu **BASE-5** (varem esindama kuue kajastatava perioodi) ja käivitada aruande, mis sisaldab baasperioodi 2. Sel juhul kuvatakse aruandes andmeid ka kahe esimese määratud aasta ja neli eelmise aasta.
...
3.  Lõpetage valem navigeerimispaani kohal valemiribal või lahtris **Periood**. Asenda iga märk (\#) on sobiv väärtus.
...
Dimensiooni filtrit kasutatakse veeru **FD** piiramiseks teatud dimensiooniväärtusteni. Filtri võib sisaldada ühte dimensiooni, dimensioonide vahemikku või dimensioonide gruppi. Filter võib sisaldada ka dimensiooniväärtuste kogumeid. Kuna dimensiooniväärtusi võivad erineda, on... \financial-dimensions\dimension-Based ei ole vastavuses täpseks. Filtrit rakendatakse olenemata sellest, kas aruanne sisaldab aruandluspuud või mitte. Kasutage metamärki (\* või?) igas asendis. Kui määrate mitu kontot, panna koma vaheline, nagu järgmises näites: + konto =\[1200\], + konto =\[1100\], osakonna =\[01?\] Kõikide kindla konto osakondade saamiseks saate osakonna dimensiooni dimensioonifiltrist välistada. Näiteks käsitletakse mõlemat järgmist dimensioonifiltrit samamoodi.

-   + Konto =\[1100\], osakonna
-   + Konto =\[1100\]

Täpseks vastendamiseks saate kasutada ka mis tahes kirjamärkide kombinatsiooni ja määratleda osalised dimensioonid. Näiteks **asukoht = \[10\*\]** sisaldab kõik asukoht väärtusi algab 10.
...
-   **Valuuta Kuva** – saate määrata valuuta, (loomulik, funktsionaalne või aruandluse) tehingud kuvatakse. See funktsioon on mõnikord viidatud kui realiseerimata. Valuuta teisendamine on pearaamatu summade aruandluse võimalus valuutas, mis ei pruugi olla ettevõtte ametlik valuuta või kandele sisestatud valuuta.
...
**Märkus.** Siin kirjeldatavad suvandid võivad olenevalt ERP süsteemist erineda. Lisateabe saamiseks vaadake oma [Microsoft ERP süsteemi dokumentatsioon](https://www.microsoft.com/en-us/download/details.aspx?id=5916).
...
| Ettevõtte teabe ametlik valuuta | Tühi                | **$2,310\*\*** – tulemus näitab kõiki andmeid kasutatavas valuutas, mis määratakse vormil ettevõtte andmed. **Märkus.** See summa on kõigi kannete summa ametlikus valuutas. |
...
Saate luua veeru, mis kuvaks määratud veerus kõik väärtused baasarvu protsendina. Seega saate kuvada ridadevahelised seosed, nagu müügi rea protsendi või kogukulude rea protsendi. Korrutada või jagada iga rida baasi Real kindlas veerus, sisestage veeru arvutamisel kasutada, ja seejärel sisestage **\*BASEROW** või **/BASEROW**. Sisestage **C\*BASEROW** või **C/BASEROW**. **Märkus.** Veeru definitsioonis baasrea arvutuse kasutamisel veenduge, et iga selle veeru definitsiooniga kasutatav readefinitsioon sisaldaks arvutuste puhul vähemalt ühte baasrida.
